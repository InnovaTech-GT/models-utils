...
- **Cycle 4 insights**: `c4a_insights_dashboards` (insight dashboard/chart tables) → … → **Insights v2**: `iv1_insights_v2` (**head**, on `dc1_category_trim`, hand-written, additive): `insightcharttype` += `LINE` (autocommit block), `insight_dashboard.default_time_range` + `insight_chart.viz` JSON NULL, post-assertions; `downgrade()` drops the two columns, keeps the label, refuses while a chart uses `LINE`
