...
pytest -v                                         # 375 tests, in-memory SQLite (needs placeholder POSTGRES_* env)
...
- `alembic/` — 78 revisions (head: `nc1d_acs_tenant_credentials`, on `iv1_insights_v2` ← `dc1_category_trim` ← `fg1_integration_enabled_regby` ← `ng2_provisioning_run_list` ← `nat3_pylon_socks5` ← `nat2_gateway_host_check` ← `nat1_gateway_transport` ← `ng2_topology_drop` ← `ng1_network_graph` ← `lc2_retire_susp_react`); `env.py` imports all model modules and runs seeds after upgrade
...
- `tests/` — 38 files, 375 tests (`asyncio_mode = auto`, SQLite); `conftest.py` holds the shared `db` + `plant` fixtures (a real in-memory graph, not fakes)
...
- Not all migrations are reversible: `c1e_install_actions` uses `ALTER TYPE ... ADD VALUE` (no downgrade), and `iv1_insights_v2` keeps its `LINE` label on downgrade (its `downgrade()` refuses while a chart still uses `LINE`), and `ng2_topology_drop`'s `downgrade()` raises `NotImplementedError` on purpose.
