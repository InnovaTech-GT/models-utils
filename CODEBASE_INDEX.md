...
(`PaginatedResponse[T]`); **Cycle 4**: insight (v2: opaque `spec`/`viz`/`default_time_range`, `InsightChartSpec` deleted); **Cycle 5**: acs_registration,
...
**Tenant ACS credentials** (nc1d, 2026-09-23) adds no modules — `NetworkAccessOut` gains read-only `acs_username` + `has_acs_password`; `NetworkAccessCreate`/`NetworkAccessUpdate` accept neither (backend-erp issues the credentials).
...
- **Cycle 4 insights**: `c4a_insights_dashboards` (insight dashboard/chart tables) → … → **Insights v2**: `iv1_insights_v2` (on `dc1_category_trim`, hand-written, additive): `insightcharttype` += `LINE` (autocommit block), `insight_dashboard.default_time_range` + `insight_chart.viz` JSON NULL, post-assertions; `downgrade()` drops the two columns, keeps the label, refuses while a chart uses `LINE`
- **Tenant ACS credentials**: `nc1d_acs_tenant_credentials` (**head**, on `iv1_insights_v2`, additive): `network_access.acs_username` (globally unique) + `network_access.acs_password_hash` (bcrypt), CHECK `kind='acs'` only; `downgrade()` drops both columns and both constraints
...
- **Cycle 10 network graph (doc 35)**: `ng1_network_graph` → `ng2_topology_drop`, both on `lc2_retire_susp_react`.
