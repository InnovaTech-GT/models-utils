...
- `alembic/` — 60 revisions (head: `dc1_category_trim`, on `fg1_integration_enabled_regby` ← `ng2_provisioning_run_list` ← `nat3_pylon_socks5` ← `nat2_gateway_host_check` ← `nat1_gateway_transport` ← `ng2_topology_drop` ← `ng1_network_graph` ← `lc2_retire_susp_react`); `env.py` imports all model modules and runs seeds after upgrade
