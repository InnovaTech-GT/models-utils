...
| `NetworkAccess` | `network_access` | name, kind (CHECK: acs\|olt), mode (CHECK: direct\|vpn\|tunnel\|nat_zt\|nat_public), is_default, mgmt_subnets (JSON CIDRs), acs_base_url, **`gateway_host`** (String, nullable — NAT transport, 2026-08-13), **`pylon_socks5`** (String, nullable — per-tenant Pylon SOCKS5 endpoint, 2026-08-17), **`acs_username`** (String, nullable, globally unique — tenant TR-069 inform username, 2026-09-23), **`acs_password_hash`** (String, nullable, bcrypt — both CHECK-restricted to `kind='acs'`) | Per-tenant transport config (canon C9). `mgmt_subnets`-based longest-prefix match was never implemented; `transport.py::resolve_endpoint` reads only the default `kind='olt'` row — see below |
...
## Tenant TR-069 inform credentials (nc1d, 2026-09-23)

Each tenant's ACS transport row (`network_access`, `kind='acs'`) carries the
credentials its CPEs present to our ACS over HTTP Basic on every CWMP session:
`acs_username` (generated by backend-erp from the company, globally unique via
`uq_network_access_acs_username`, so the username alone identifies the tenant)
and `acs_password_hash` (bcrypt via `database_utils.utils.password`). Both stay
NULL until issued, and `ck_network_access_acs_credentials_kind` rejects them on
any non-`acs` row.
