...
# Tenant TR-069 inform credentials only make sense on the ACS transport row.
# The router enforces it too, but a direct UPDATE could bypass it, so the DB
# closes that path (same layering as the NAT checks above).
...
class NetworkAccess(Base):
    ...
    # Tenant-scoped TR-069 inform credentials (CPE -> ACS, HTTP Basic auth on
    # every CWMP session). Unlike device_credential (secrets WE use to dial
    # devices, envelope-encrypted per canon C1), this credential is presented
    # BY the device TO us, so only a bcrypt hash is stored
    # (database_utils.utils.password): it is compared, never read back.
    # acs_username is generated by the backend from the company and is
    # globally unique, so the username alone identifies the tenant.
    # Both NULL until credentials are issued; kind='acs' rows only (CHECK).
    acs_username = Column(String, nullable=True)
    acs_password_hash = Column(String, nullable=True)
    ...
    @property
    def has_acs_password(self) -> bool:
        """API-safe indicator (mirrors DeviceCredential.has_secret): the
        bcrypt hash itself is never serialized."""
        return self.acs_password_hash is not None

    __table_args__ = (
        UniqueConstraint("company_id", "name", name="uq_network_access_company_name"),
        # Global, not per company: the username a CPE presents is what tells us
        # which tenant it belongs to. NULLs are distinct in both Postgres and
        # SQLite, so rows without credentials never collide.
        UniqueConstraint("acs_username", name="uq_network_access_acs_username"),
        ...
        CheckConstraint(
            _NETWORK_ACCESS_ACS_CREDENTIALS_CHECK,
            name="ck_network_access_acs_credentials_kind",
        ),
